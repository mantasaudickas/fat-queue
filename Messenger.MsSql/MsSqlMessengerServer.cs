...
{
    public class MsSqlMessengerServer : CommonComponent, IMessengerServer
    {
        ...
        public MsSqlMessengerServer(MsSqlServerSettings settings)
            : base(settings)
        {
        ...
        public void Start(CancellationToken? cancellationToken)
        {
            ...
            startLogMessage.AppendFormat("Starting MsSqlMessengerServer with {0} processes.", _maxProcessCount).AppendLine();
            ...
            {
                CleanupService cleanupService = new CleanupService(_connectionString, Logger);
        ...
        private void StartMonitor(CancellationToken cancellationToken)
        {
            ...
            TaskMonitoringService monitoringService = new TaskMonitoringService(_connectionString, Logger, tasks);
        ...
        private string GetProcessName(int id)
        {
            ...
            switch (_processNameFormat)
            {
                case ProcessNameFormat.ByMachineNameAndApplicationDirectory:
                    var directory = Path.GetDirectoryName(typeof (MsSqlMessengerServer).Assembly.Location);
        ...
        private void ProcessMessages(CancellationToken cancelationToken, string processName)
        {
            ...
            var processor = new MessageProcessor(_connectionString, _jobActivator, Serializer, Logger);
