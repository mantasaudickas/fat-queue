...
{
    public class FatQueueSignalREventHandler : IHandler<FatQueueSignalREvent>
    {
        public void Handle(FatQueueSignalREvent request)
        {
                Task task = new Task(ExecAsync);
                task.Start();
                task.Wait();
        }
        ...
        private void ExecAsync()
        {
            using (var connection = new SqlConnection("Server=.\\SQLEXPRESS;Database=Messenger;Integrated Security=SSPI"))
            {
                ...
                using (var command = connection.CreateCommand())
                {
                    ...
                    var parameter = command.Parameters.Add("@PayLoad", SqlDbType.VarBinary);
                    parameter.Value = Encoding.UTF8.GetBytes("Hello world");
        ...
        private string GetSql()
        {
            return "BEGIN TRANSACTION " +
                   "INSERT INTO [SignalR].[Messages_0] ([PayloadId], [Payload], [InsertedOn]) VALUES(1, @Payload, GETDATE()) " +
                   "COMMIT";

            return @"-- Params: @Payload varbinary(max)
